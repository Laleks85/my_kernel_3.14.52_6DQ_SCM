...
/*****************************************************************************************/
/*                        Test bus_freq from user space                                  */
/*****************************************************************************************/

static ssize_t bus_freq_set(struct device *dev,
                                struct device_attribute *attr, const char *buf, size_t size)
{

        enum bus_freq_mode bus_rate = BUS_FREQ_HIGH;
	char command[8];
	
        sscanf(buf, "%s %d" ,  &command[0], &bus_rate);

	if( 0 == strncmp(command, "request", 7) )
        request_bus_freq(bus_rate);
	else if( 0 == strncmp(command, "release", 7) )
	release_bus_freq(bus_rate);

        return size;
}

static ssize_t bus_freq_read(struct device *dev,
                                 struct device_attribute *attr,
                                  char *buf)
{
        return sprintf(buf, "High Bus count : %d\n Med Bus count : %d\n Audio Bus count : %d\n Low Bus count : %d\n", high_bus_count, med_bus_count, audio_bus_count, low_bus_count);
}




static DEVICE_ATTR(freq, 0644, bus_freq_read,
                        bus_freq_set);
...
static int busfreq_probe(struct platform_device *pdev)
{
	...
	err = sysfs_create_file(&busfreq_dev->kobj, &dev_attr_freq.attr);
	if (err) {
		dev_err(busfreq_dev,
		       "Unable to register sysdev entry for BUSFREQ");
		return err;
